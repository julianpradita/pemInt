...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 09, 2015 at 08:52 AM
...
CREATE TABLE IF NOT EXISTS `lowongan` (
  `IdLowongan` char(5) NOT NULL,
  `IdPerusahaan` char(5) NOT NULL,
  `Kualifikasi` text NOT NULL,
  `JenisLowongan` enum('Tetap','Kontrak','FreeLance') NOT NULL,
  `Klasifikasi` text NOT NULL,
  `Judul` varchar(40) NOT NULL,
  `Deskripsi` text NOT NULL,
  `tanggal` datetime NOT NULL,
  `Status` enum('Buka','Tutup','Tunggu') NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `pencari` (
  `IdUser` char(5) NOT NULL,
  ...
  `Didik` enum('SD/MI','SMP/MTS','SMA/SMK/MA','D3','S1','S2','S3') NOT NULL,
...
ALTER TABLE `kabupaten`
 ADD PRIMARY KEY (`kota_id`), ADD KEY `master_kokab` (`provinsi_id`);
...
--
ALTER TABLE `kabupaten`
ADD CONSTRAINT `master_kokab` FOREIGN KEY (`provinsi_id`) REFERENCES `provinsi` (`provinsi_id`);
